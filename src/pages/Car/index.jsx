...
import { getCars,deleteCar } from '@/services/car';
...
const Index = () => {
    ...
    const deleteCars = async(params)=>{
        const response = await deleteCar(params);
        if(response.status ==='ok'){
            message.success('删除成功');
            actionRef.current.reload();
        }
        else{
            message.error('删除失败,请检查路线中是否含有该汽车信息！');
        }
    }
    ...
    const columns = [
        ...
        },
        {
            title: '编辑',
            hideInSearch: true,
            render: (_, record) => <a onClick={() => isShowModalEdit(true, record.CarNUM)}>编辑</a>
        },
        {
            title: '删除',
            hideInSearch: true,
            render: (_, record) => <a onClick={() => deleteCars(record.CarNUM)}>删除</a>
        },
    ];
    return (
        <PageContainer>
